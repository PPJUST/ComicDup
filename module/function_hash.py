...
def calc_image_hash(image_file, calc_hash: str = 'all'):
    ...
    image_pil = image_pil.resize(size=(RESIZE_IMAGE_ACCURACY, RESIZE_IMAGE_ACCURACY))

    # 均值哈希
    if calc_hash == 'ahash' or calc_hash == 'all':
        hash_a = imagehash.average_hash(image_pil, hash_size=RESIZE_IMAGE_ACCURACY)
        hash_a_str = _hash_numpy2str(hash_a)
        pic_hash_dict['ahash'] = hash_a_str
    # 感知哈希
    if calc_hash == 'phash' or calc_hash == 'all':
        hash_p = imagehash.phash(image_pil, hash_size=RESIZE_IMAGE_ACCURACY)
        hash_p_str = _hash_numpy2str(hash_p)
        pic_hash_dict['phash'] = hash_p_str
    # 差异哈希
    if calc_hash == 'dhash' or calc_hash == 'all':
        hash_d = imagehash.dhash(image_pil, hash_size=RESIZE_IMAGE_ACCURACY)
